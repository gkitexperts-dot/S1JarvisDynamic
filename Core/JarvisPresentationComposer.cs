...
namespace S1Jarvis.Core
{
    ...
    /// <summary>
    /// Presentation-only AI boundary. It may choose wording, but it never queries
    /// Soft1, changes validated rows, or executes actions. Large datasets are not
    /// sent back to the model: only schema/count and a small sample are provided.
    /// </summary>
    internal static class JarvisPresentationComposer
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            JObject request = BuildRequest(
                ...
                "Το email body πρέπει να είναι έτοιμο για πραγματική αποστολή και να έχει ανθρώπινη μορφή, όχι raw key=value dump. " +
                "Επέστρεψε ΑΠΟΚΛΕΙΣΤΙΚΑ JSON: {\"intro\":\"σύντομη φράση πριν το draft\",\"emailBody\":\"...\"}.");
        ...
        internal static string BuildMarkdownTable(string datasetJson, int maxRows)
        {
            try
            {
                ...
                if (rows == null || rows.Count == 0)
                    return "_Δεν βρέθηκαν εγγραφές._";
        ...
        private static JObject BuildCompactDatasetContext(string businessQuestion, string datasetJson)
        {
            ...
            try
            {
                ...
                foreach (JObject row in rows.OfType<JObject>().Take(5))
                    samples.Add(row.DeepClone());
                context["sampleRows"] = samples;
            }
            catch
            {
                context["datasetParseError"] = true;
            }
            return context;
        }
                ...
                ["system"] = new JArray
                {
                    new JObject
                    {
                        ...
                        ["text"] = "Είσαι ο Jarvis presentation layer. Τα δεδομένα που λαμβάνεις έχουν ήδη επικυρωθεί από τον Jarvis control plane. " +
                                   "Επιτρέπεται να αλλάξεις μόνο wording/μορφοποίηση. Απαγορεύεται να αλλάξεις business facts, να κάνεις query, να συμπληρώσεις ελλείποντα στοιχεία ή να εκτελέσεις action. " +
                                   instruction
                    }
                },
                ["messages"] = new JArray
