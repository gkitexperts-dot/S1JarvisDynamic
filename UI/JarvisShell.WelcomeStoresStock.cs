...
namespace S1Jarvis.UI
{
    public partial class JarvisShell
    {
            ...
            CoreWebView2NavigationCompletedEventArgs e)
        {
            ...
            try
            {
                await webView.CoreWebView2.ExecuteScriptAsync(BuildWelcomeStoresStockScript());
        ...
        private static string BuildWelcomeStoresStockScript()
        {
            return @"
(function(){
  ...
  function open(){ curtain.classList.add('open'); setTimeout(()=>search.focus(),20); }
  function close(){ curtain.classList.remove('open'); }
  ...
  function runSearch(){
    ...
    items.innerHTML=''; selected.style.display='none';
  ...
  window.welcomeStoresStockReceive=function(kind,data){
    if(kind==='error'||!data||data.success===false){ status.textContent=(data&&data.message)||'Σφάλμα.'; return; }
    if(kind==='search'){
      status.textContent=data.items&&data.items.length?('Βρέθηκαν '+data.items.length+' είδη.'):'Δεν βρέθηκαν είδη.';
      items.innerHTML='';
      (data.items||[]).forEach(item=>{const b=document.createElement('button');b.className='ws-item';b.innerHTML='<strong>'+esc(item.Code)+'</strong><span>'+esc(item.Name)+'</span>';b.addEventListener('click',()=>chooseItem(item));items.appendChild(b);});
      return;
    }
    if(kind==='availability'){
      const list=data.rows||[];
      status.textContent=list.length?('Διαθεσιμότητα σε '+list.length+' γραμμές αποθήκης.'):'Δεν βρέθηκε διαθέσιμο απόθεμα.';
      rows.innerHTML='';
      if(!list.length){rows.innerHTML='<tr><td colspan="9" class="ws-empty">Δεν υπάρχει διαθέσιμο απόθεμα πάνω από το όριο.</td></tr>';return;}
      list.forEach(r=>{
        const state=r.IsCurrentStore?'<span class="ws-badge current">Τρέχον κατάστημα</span>':(r.SupplierExists?'<span class="ws-badge ok">✓ Προμηθευτής</span>':'<button class="ws-btn secondary" disabled>Άνοιγμα</button>');
        const action=r.IsCurrentStore?'<span style="color:#777">—</span>':'<button class="ws-btn" disabled>Παραγγελία</button>';
        const tr=document.createElement('tr');
        tr.innerHTML='<td>'+esc(r.StoreName)+'</td><td>'+esc(r.Afm)+'</td><td>'+esc(r.Phone)+'</td><td>'+esc(r.ItemCode)+'</td><td>'+esc(r.ItemName)+'</td><td>'+esc(r.Available)+'</td><td>'+state+'</td><td>'+(r.IsCurrentStore?'—':'<input class="ws-qty" type="number" min="1" max="'+esc(r.Available)+'" value="1">')+'</td><td>'+action+'</td>';
        rows.appendChild(tr);
      });
    }
  };
  ...
  if(mainInput) mainInput.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey)interceptStock(e);},true);
})();";
        }
    }
}
