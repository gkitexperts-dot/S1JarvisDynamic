(function(){
  ...
  var DR_LINE_STATE={Pending:0,Extracted:10,ExactMatched:20,SupplierMapped:30,Proposed:40,ManualResolved:50,NewItemCreated:60,NeedsReview:70,Blocked:80,Ready:90,Registered:100};
  ...
  style.textContent=`
    ...
    .dr-session-complete-icon{color:#8fe0b7;font-size:15px;font-weight:700;flex:none}.dr-session-summary{flex:1;min-width:0}
  ...
  function ensureBatchStartControl(files){
    var list=document.getElementById('drFileList');if(!list)return;
    var host=list.parentElement||list,control=host.querySelector('[data-dr-batch-start]');
    var pending=files.filter(function(f){return !done(f);});
    if(window.__jarvisDrBatchStarted||!pending.length){if(control)control.remove();return;}
    if(!control){control=document.createElement('div');control.className='dr-batch-start';control.setAttribute('data-dr-batch-start','1');if(list.parentNode)list.parentNode.insertBefore(control,list);}
    control.innerHTML='<div class="dr-batch-start-copy"><div class="dr-batch-start-title">'+pending.length+' παραστατικ'+(pending.length===1?'ό':'ά')+' στον κουβά</div><div class="dr-batch-start-meta">Πρόσθεσε όσα αρχεία θέλεις και ξεκίνα όταν είσαι έτοιμος.</div></div><button type="button" class="dr-batch-start-btn" data-dr-start-recognition>Έναρξη αναγνώρισης</button>';
  }

  window.renderDrFileList=function(){var files=visibleFiles();files.forEach(syncLineRecognitionStates);if(!files.length){window.__jarvisDrBatchStarted=false;ensureBatchStartControl([]);baseRenderDrFileList();return;}var completed=files.filter(done),active=chooseActive(files),queued=window.__jarvisDrBatchStarted?(active?files.filter(function(f){return !done(f)&&f!==active;}):[]):files.filter(function(f){return !done(f);});var html='<div class="dr-session-stack">';completed.forEach(function(f){html+=completedRow(f);});if(active)html+=activeBlock(active);queued.forEach(function(f,i){html+=queuedRow(f,i+1,!window.__jarvisDrBatchStarted);});html+='</div>';drFileListEl.innerHTML=html;ensureBatchStartControl(files);if(active&&typeof setDrActiveFile==='function'&&String(drActiveFileId)!==String(active.id))setDrActiveFile(active);setTimeout(function(){moveAssistant(active);},0);};

  document.addEventListener('click',function(ev){var start=ev.target.closest('[data-dr-start-recognition]');if(start){var pending=visibleFiles().filter(function(f){return !done(f);});if(!pending.length)return;window.__jarvisDrBatchStarted=true;window.renderDrFileList();if(typeof window.refreshDrRecognitionWorkspace==='function')setTimeout(window.refreshDrRecognitionWorkspace,0);return;}var btn=ev.target.closest('[data-dr-open-completed]');if(!btn)return;var id=btn.getAttribute('data-dr-open-completed'),f=visibleFiles().find(function(x){return String(x.id)===String(id);});if(!f||!f.registerResult||!f.registerResult.findocId)return;var d=docInfo(f),r=f.registerResult||{},sosource=r.sosource||firstValue(d,['sosource','SOSOURCE'])||f.manualSosource||(f.seriesGuess&&f.seriesGuess.sosource);if(!sosource){alert('Το παραστατικό δημιουργήθηκε, αλλά δεν υπάρχει διαθέσιμο SOSOURCE για άνοιγμα από αυτή την προβολή.');return;}postCommand({type:'courier_open_document',sosource:Number(sosource),mode:'locate',id:Number(r.findocId)});},true);
