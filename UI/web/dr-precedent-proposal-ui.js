(function(){
  ...
  var style=document.createElement('style');style.id='drPrecedentProposalUiStyle';style.textContent=`
    ...
    .dr-precedent-confirm{margin-top:8px;border:1px solid rgba(76,201,138,.42);background:rgba(76,201,138,.16);color:#dff8ea;border-radius:7px;padding:6px 10px;font-size:10.5px;font-weight:650;cursor:pointer;transition:opacity .15s ease,transform .08s ease}.dr-precedent-confirm:active{transform:translateY(1px)}.dr-precedent-confirm:disabled{opacity:.58;cursor:wait}
    .dr-precedent-feedback{margin-top:7px;font-size:10px;color:#aaa9bd}.dr-precedent-feedback.error{color:#ffb1b1}.dr-precedent-feedback.ok{color:#8fe0b7}
  ...
  document.addEventListener('click',function(e){
    ...
    var confirm=e.target.closest('[data-dr-confirm-precedent-mapping]');if(!confirm)return;
    e.preventDefault();e.stopImmediatePropagation();
    var f=active(),p=f&&f.precedentMappingProposal;if(!f||!p||p.status!=='proposed'||!p.target||!p.target.mtrlId)return;
    p.status='writing';p.feedback='Αποθήκευση CCCMAPITEMS σε εξέλιξη…';rerender(f);setTimeout(refresh,0);
    try{
      if(typeof postCommand!=='function')throw new Error('postCommand_not_available');
      postCommand({type:'dr_confirm_precedent_mapping',fileId:String(f.id),confirm:true,trdrId:Number(f.trdrId),findocId:Number(p.findocId),targetMtrlId:Number(p.target.mtrlId),mappings:p.tokens||[]});
      setTimeout(function(){if(p.status==='writing'){p.status='blocked';p.errorMessage='Δεν ελήφθη απάντηση από το Soft1 για την αποθήκευση mappings.';p.feedback='Timeout κατά την αποθήκευση CCCMAPITEMS.';rerender(f);setTimeout(refresh,0);}},15000);
    }catch(err){p.status='blocked';p.errorMessage=String(err&&err.message||err);p.feedback='Το command δεν στάλθηκε.';rerender(f);setTimeout(refresh,0);}
  },true);
  ...
  if(window.chrome&&window.chrome.webview)window.chrome.webview.addEventListener('message',function(ev){
    ...
    if(!d.success){p.status='blocked';p.errorMessage=d.errorMessage||d.reason||'Αποτυχία αποθήκευσης CCCMAPITEMS.';p.feedback='Το Soft1 απέρριψε ή δεν ολοκλήρωσε την αποθήκευση.';rerender(f);setTimeout(refresh,0);return;}
