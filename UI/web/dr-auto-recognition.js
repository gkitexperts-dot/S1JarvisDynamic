(function(){
  ...
  function analyzePosting(f){
    ...
    f.postingProposalRequested=true;f.postingProposal={success:true,resolver:'resolve_document_pattern',version:3,mode:'Unknown',needsReview:true,reason:'resolving_historical_pattern',sourceLineCount:lines.length};rerender(f);
  ...
  async function tick(){var f=active();if(!f)return;if(f.status==='pending'){await identify(f);return;}if(f.status==='identified'&&!f.traderRolesResolved&&!f.traderRolesRequested){requestTraderRoles(f);return;}if(f.status==='identified'&&f.traderRolesResolved&&!f.extraction&&!f.linesExtracting){await extract(f);return;}if(f.extraction&&!f.postingProposalRequested&&(!f.postingProposal||f.postingProposal.reason==='resolving_historical_pattern'))analyzePosting(f);}
