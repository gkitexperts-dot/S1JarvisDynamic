...
(function(){
  ...
  var initialSlot=document.querySelector('.dr-session-active .dr-session-chat-slot'); if(initialSlot)initialSlot.appendChild(panel); else host.appendChild(panel);
  ...
  if(typeof window.setDrActiveFile==='function'){var original=window.setDrActiveFile;window.setDrActiveFile=function(entry){var r=original.apply(this,arguments);renderHistory(entry);return r;};}
  ...
  renderHistory(activeEntry()); window.__jarvisDrAssistantInstalled=true;
})();";
